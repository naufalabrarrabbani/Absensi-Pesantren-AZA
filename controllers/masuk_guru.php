...
if(mysqli_num_rows($cek_guru) == 0) {
    header('location:../masuk_guru.php?status=gagal&pesan='.base64_encode('NIP Guru tidak terdaftar!'));
    exit();
}
...
if(mysqli_num_rows($cek_absen) > 0) {
    ...
    if($data_absen['masuk'] != null) {
        // Sudah absen masuk
        header('location:../masuk_guru.php?status=sudah&pesan='.base64_encode('Anda sudah melakukan absen masuk hari ini pada pukul '.date('H:i', strtotime($data_absen['masuk']))).'&nama='.base64_encode($data_guru['nama']));
    } else {
        // Update jam masuk
        $update = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE absensi_guru SET masuk='$jam' WHERE nip='$nip' AND tanggal='$tanggal'");
        if($update) {
            header('location:../masuk_guru.php?status=berhasil&pesan='.base64_encode('Absen masuk berhasil!').'&nama='.base64_encode($data_guru['nama']));
        } else {
            header('location:../masuk_guru.php?status=gagal&pesan='.base64_encode('Gagal melakukan absen masuk!'));
        }
    }
} else {
