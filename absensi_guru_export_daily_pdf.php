...
// Query for daily teacher attendance data
$query = "SELECT g.nip, g.nama, g.mata_pelajaran, g.jabatan, g.foto, 
...
while ($data = mysqli_fetch_array($result)) {
    ...
    $attendance_data[] = [
        'nip' => $data['nip'],
        'nama' => $data['nama'],
        'mata_pelajaran' => $data['mata_pelajaran'] ?: 'Tidak ada mata pelajaran',
...
$filename = 'Laporan_Absensi_Guru_Harian_' . date('d-m-Y', strtotime($selected_date)) . '.html';
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Laporan Absensi Guru Harian - <?= date('d F Y', strtotime($selected_date)) ?></title>
    <style>
        @media print {
            @page { 
                size: A4; 
...
<body>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-size: 7px;"><?= htmlspecialchars($data['mata_pelajaran']) ?></td>
                <td style="font-size: 7px;"><?= htmlspecialchars($data['jabatan']) ?></td>
                <td class="text-center"><?= $data['masuk'] ?></td>
                <td class="text-center"><?= $data['pulang'] ?></td>
                <td class="text-center <?= $status_class ?>"><?= $data['status'] ?></td>
                <td style="font-size: 6px;"><?= htmlspecialchars($data['keterangan']) ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <table class="stats-table">
        ...
        <tbody>
            ...
            <tr>
                <td>Hadir</td>
                <td class="status-hadir"><?= $total_present ?> guru (<?= $total_teachers > 0 ? round(($total_present / $total_teachers) * 100, 1) : 0 ?>%)</td>
            </tr>
            <tr>
                <td>Sakit</td>
            ...
            <tr style="background-color: #ffebee;">
                ...
                <td><?= ($total_sick + $total_permission + $total_alpha) ?> guru (<?= $total_teachers > 0 ? round((($total_sick + $total_permission + $total_alpha) / $total_teachers) * 100, 1) : 0 ?>%)</td>
            </tr>
            <tr style="background-color: #e8f5e8;">
                <td>Persentase Kehadiran</td>
                <td class="status-hadir"><?= $total_teachers > 0 ? round(($total_present / $total_teachers) * 100, 1) : 0 ?>%</td>
            </tr>
        </tbody>
    </table>
